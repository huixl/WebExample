...
public class LoginServiceImpl implements LoginService {
    ...
    public boolean verifyCurrentPassword(PasswordVerifyRequest passwordVerifyRequest) {
        AccountPo currentUser = getCurrentUser();
        if (currentUser == null) {
     ...
     */
    @Override
    public List<AccountPo> getAllUsers() {
        LambdaQueryWrapper<AccountPo> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(AccountPo::getVerificationStatus, 1)
                .orderByDesc(AccountPo::getCreateTime);
